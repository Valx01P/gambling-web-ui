...
const SideBetsPanel = memo(function SideBetsPanel({
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden">
      <div className="flex shrink-0 items-center justify-between border-b border-zinc-700/60 bg-zinc-900/60 px-3 py-2">
        <div className="flex items-baseline gap-2">
          <span className="text-[10px] font-black uppercase tracking-widest text-amber-300">Side Bets</span>
          <span className="text-[10px] text-zinc-500">vs. House · 4% edge</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-[10px] font-bold text-zinc-400">{liveCount} live</span>
          {onToggleExpanded && (
            <button
              type="button"
      ...
      <div className="flex-1 overflow-y-auto overscroll-contain px-2 py-2 space-y-2">
        {props.length === 0 && (
          <div className="px-2 py-4 text-center text-xs italic text-zinc-500">
            Side bets spawn when a hand starts.
          </div>
        )}
        {props.map(prop => (
          <PropCard
...
const PropCard = memo(function PropCard({ prop, myPositions, myStack, volume, onPlace, onSell, expanded: panelExpanded }) {
  ...
  // outcome). Stops the dock from filling up with 5+ identical-shape "NO"
  // cards after a hand finishes. Skipped when the user is in the expanded
  // overlay — there they get the full card regardless.
  if (isResolved && myPositions.length === 0 && !panelExpanded) {
    return (
      <div
      ...
      >
        <div className="min-w-0 flex-1 truncate text-[11px] text-zinc-300">
          <span className="mr-1.5 text-[9px] font-black uppercase tracking-widest text-zinc-500">
            {prop.shortLabel}
          </span>
          {prop.question}
        </div>
